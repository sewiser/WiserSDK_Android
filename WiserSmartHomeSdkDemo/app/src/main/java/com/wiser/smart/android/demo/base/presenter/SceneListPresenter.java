...
import com.wiser.smart.android.demo.scene.activity.SceneActivity;
...
import com.wiser.smart.android.demo.scene.view.ISceneListFragmentView;
...
public class SceneListPresenter extends BasePresenter {
    ...
    public void getSceneList(){
        WiserHomeSdk.getSceneManagerInstance().getSceneList(Constant.HOME_ID, new IWiserResultCallback<List<SceneBean>>() {
            @Override
    ...
    public void execute(SceneBean bean) {
        WiserHomeSdk.newSceneInstance(bean.getId()).executeScene(new IResultCallback() {
            @Override
    ...
    public void switchAutomation(SceneBean bean) {
        if(bean.isEnabled()){
            WiserHomeSdk.newSceneInstance(bean.getId()).disableScene(bean.getId(), new IResultCallback() {
                @Override
                public void onError(String code, String error) {
            ...
            WiserHomeSdk.newSceneInstance(bean.getId()).enableScene(bean.getId(), new IResultCallback() {
                @Override
                public void onError(String code, String error) {
                    ToastUtil.shortToast(mActivity, error);
                }
