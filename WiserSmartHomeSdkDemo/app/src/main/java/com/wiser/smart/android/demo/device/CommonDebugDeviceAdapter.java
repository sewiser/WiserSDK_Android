...
import com.wiser.smart.android.demo.base.adapter.BaseListArrayAdapter;
import com.wiser.smart.android.demo.base.adapter.ViewHolder;
    ...
    private static class InnerViewHolder extends ViewHolder<SchemaBean> {
        ...
        private static String getProperty(SchemaBean schemaBean) {
            ...
            if (TextUtils.equals(mode, ModeEnum.RO.getType())) {
                modeType = WiserSdk.getApplication().getString(R.string.mode_only_read);
            } else if (TextUtils.equals(mode, ModeEnum.RW.getType())) {
                modeType = WiserSdk.getApplication().getString(R.string.mode_read_write);
            } else {
                modeType = WiserSdk.getApplication().getString(R.string.mode_only_write);
            }
            ...
            if (schemaBean.getType().equals(DataTypeEnum.OBJ.getType())) {
                ...
                String schemaType = schemaBean.getSchemaType();
                if (TextUtils.equals(schemaType, StringSchemaBean.type)) {
                    schemaTypeStr = WiserSdk.getApplication().getString(R.string.stringType);
                } else if (TextUtils.equals(schemaType, ValueSchemaBean.type)) {
                    schemaTypeStr = WiserSdk.getApplication().getString(R.string.valueType);
                } else if (TextUtils.equals(schemaType, BitmapSchemaBean.type)) {
                    schemaTypeStr = WiserSdk.getApplication().getString(R.string.bitmapType);
                } else if (TextUtils.equals(schemaType, BoolSchemaBean.type)) {
                    schemaTypeStr = WiserSdk.getApplication().getString(R.string.booleanType);
                } else if (TextUtils.equals(schemaType, EnumSchemaBean.type)) {
                    schemaTypeStr = WiserSdk.getApplication().getString(R.string.enumType);
                }
            } else if (schemaBean.getType().equals(DataTypeEnum.RAW.getType())) {
                //raw | file 类型
                schemaTypeStr = WiserSdk.getApplication().getString(R.string.rawType);
            }
