...
import com.wiser.smart.android.demo.base.utils.SchemaUtil;
...
public class DpSendPresenter extends BasePresenter implements IDevListener {
    ...
    private void initData() {
        ...
        Map<String, SchemaBean> schema = WiserHomeSdk.getDataInstance().getSchema(mDevId);
        if (schema != null) {
            mSchemaBean = schema.get(mDpId);
        }
        mDev = WiserHomeSdk.getDataInstance().getDeviceBean(mDevId);
        if (mDev == null) {
    ...
    public void sendDpValue(final Object dps) {
        ...
        mWiserDevice.publishDps(value, new IResultCallback() {
            @Override
