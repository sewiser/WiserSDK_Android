...
import com.wiser.smart.android.demo.login.IAccountConfirmView;
...
import com.wiser.smart.android.user.api.IRegisterCallback;
import com.wiser.smart.android.user.api.IResetPasswordCallback;
import com.wiser.smart.android.user.api.IValidateCallback;
...
import static com.wiser.smart.android.demo.login.activity.AccountConfirmActivity.MODE_REGISTER;
...
public class AccountConfirmPresenter extends BasePresenter {
        ...
        switch (mView.getPlatform()) {
            case AccountConfirmActivity.PLATFORM_EMAIL:
                if (mView.getMode() == MODE_REGISTER){
                    WiserHomeSdk.getUserInstance().getRegisterEmailValidateCode(mCountryCode,mEmail,iResultCallback);
                }else {
                    WiserHomeSdk.getUserInstance().getEmailValidateCode(mCountryCode, mEmail, mIValidateCallback);

                }
                break;

            case AccountConfirmActivity.PLATFORM_PHONE:
                WiserHomeSdk.getUserInstance().getValidateCode(mCountryCode, mPhoneNum, mIValidateCallback);
                break;
        }
    ...
    private void resetPassword() {
        switch (mView.getPlatform()) {
            case AccountConfirmActivity.PLATFORM_EMAIL:
                WiserHomeSdk.getUserInstance().resetEmailPassword(mCountryCode, mEmail, mView.getValidateCode(), mView.getPassword(), mIResetPasswordCallback);
                break;

            case AccountConfirmActivity.PLATFORM_PHONE:
                WiserHomeSdk.getUserInstance().resetPhonePassword(mCountryCode, mPhoneNum, mView.getValidateCode(), mView.getPassword(), mIResetPasswordCallback);
                break;
        }
    }
    ...
    private void register() {
        switch (mView.getPlatform()) {
            case AccountConfirmActivity.PLATFORM_EMAIL:
                WiserHomeSdk.getUserInstance().registerAccountWithEmail(mCountryCode, mEmail, mView.getPassword(), mView.getValidateCode(),mIRegisterCallback);
                break;

            case AccountConfirmActivity.PLATFORM_PHONE:
                WiserHomeSdk.getUserInstance().registerAccountWithPhone(mCountryCode, mPhoneNum, mView.getPassword(), mView.getValidateCode(), mIRegisterCallback);
                break;
        }
    }

    private void loginWithPhoneCode() {
        WiserHomeSdk.getUserInstance().loginWithPhone(mCountryCode, mPhoneNum, mView.getValidateCode(), mILoginCallback);
    }

    private void loginWithPassword() {
        if (mView.getPlatform() == AccountConfirmActivity.PLATFORM_PHONE) {
            if (mView.getMode() == AccountConfirmActivity.MODE_FORGET_PASSWORD) {
                WiserHomeSdk.getUserInstance().loginWithPhonePassword(mCountryCode, mPhoneNum, mView.getPassword(), mILoginCallback);
            } else {
                WiserHomeSdk.getUserInstance().loginWithPhonePassword(mCountryCode, mPhoneNum, mView.getPassword(), mILoginCallback);
            }

        } else {
            WiserHomeSdk.getUserInstance().loginWithEmail(mCountryCode, mEmail, mView.getPassword(), mILoginCallback);
        }
    }
