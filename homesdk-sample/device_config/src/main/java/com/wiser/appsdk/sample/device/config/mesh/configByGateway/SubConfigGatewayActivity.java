package com.wiser.appsdk.sample.device.config.mesh.configByGateway;
...
public class SubConfigGatewayActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private IWiserActivator mWiserGWSubActivator;
    ...
    protected void onDestroy() {
        super.onDestroy();
        if (mWiserGWSubActivator != null) {
            mWiserGWSubActivator.onDestroy();
        }
    }
    ...
    public void addSubDevice() {
        ...
        List<DeviceBean> deviceBeanList = WiserHomeSdk.newSigMeshDeviceInstance(mSigMeshBean.getMeshId()).getMeshSubDevList();
            ...
            for (DeviceBean deviceBean : deviceBeanList) {
                ...
                if (deviceBean.isSigMeshWifi()) {
                    ...
                    WiserGwSubDevActivatorBuilder builder = new WiserGwSubDevActivatorBuilder()
                            ...
                            .setListener(mWiserSmartActivatorListener);

                    mWiserGWSubActivator = WiserHomeSdk.getActivatorInstance().newGwSubDevActivator(builder);
                    // start add sub device
                    mWiserGWSubActivator.start();
                    break;
                }
            }
        } else {
    ...
    private final IWiserSmartActivatorListener mWiserSmartActivatorListener = new IWiserSmartActivatorListener() {
        @Override
